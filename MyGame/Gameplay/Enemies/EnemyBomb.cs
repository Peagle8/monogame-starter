...
internal sealed class EnemyBomb
{
    ...
    private bool _hasConsumedExplosion;
    ...
    public bool TryConsumeExplosion(Rectangle targetBounds, out EnemyAttack attack)
    {
        if (!IsExploding || _hasConsumedExplosion || !ExplosionBounds.Intersects(targetBounds))
        {
            attack = null!;
            return false;
        }

        _hasConsumedExplosion = true;
        attack = _attack;
        return true;
    }
}
